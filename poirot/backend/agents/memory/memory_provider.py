...
class MemoryProvider(Protocol):
    ...
    组合模型（参照 deer-flow SandboxProvider + MemoryStorage）：
    - store()     → MemoryStore    持久化后端（默认 MarkdownFileStore，truth source，总在）。
    - retriever() → Retriever      检索后端（默认 HybridRetriever，BM25 + optional Vector/Graph）。
    - manager()   → MemoryManager  四操作编排（Encode / Associate / Consolidate / Reconsolidate）。
    ...
    def store(self) -> MemoryStore:
        ...
        Returns:
            MemoryStore 实例（默认 MarkdownFileStore）。
    ...
    def retriever(self) -> Retriever:
        ...
        Returns:
            Retriever 实例（默认 HybridRetriever）。

        Raises:
            不主动抛异常。

        组装规则：
            1. 返回构造时注入的 retriever 实例。
        """
        ...
    ...
    def manager(self) -> MemoryManager:
        ...
        Returns:
            MemoryManager 实例（默认 DefaultMemoryManager）。

        Raises:
            不主动抛异常。

        组装规则：
            1. 返回构造时注入的 manager 实例。
        """
        ...