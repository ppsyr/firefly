...
class ForgetPolicy(Protocol):
    ...
    不包含矛盾解决（resolve_conflict 已删）。矛盾解决走 reconsolidate
    ...
    def should_forget(self, trace: MemoryTrace, now: float) -> bool:
        ...
        组装规则：
            1. 规则 1：TTL 过期（now - last_access > ttl_hours × 3600）→ True。
            2. 规则 2：compute_strength(trace, now) < strength_threshold → True。
            3. 两规则都未命中 → False。
        """
        ...